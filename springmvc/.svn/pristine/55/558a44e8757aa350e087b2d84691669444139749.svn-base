...
%>
<style>
#teaAddDlg_table{padding-left:20px;font-size: 12px;}
</style>
<script type="text/javascript">
	$(function(){
		...
		/*form表单提交*/
		$("#teaAddDlg_form").form({
			url :'<%=path%>/userController/saveTeacher',
			onSubmit : function() {
		...
		$('#teaAddDlg_userName').blur(function(){
            $.ajax({
...
<form id="teaAddDlg_form" method="post">
<table id="teaAddDlg_table">
	<tr>
		<td>工号</td>
		<td><input name="userName" type="text" id="teaAddDlg_userName" class="easyui-validatebox" data-options="required:true"/></td>
		<td>密码</td>
	...
	<tr>
		...
		<td>年龄</td>
		<td><input name="teaAge" type="number" class="easyui-numberbox" data-options="required:true"/>(数字格式)</td>
	</tr>	
	<tr>
		<td>部门</td>
		<td>
		...
		<td>职称</td>
		<td>
			<input name="teaTitle" id="teaTitle" type="text" class="easyui-combobox"  
    			data-options="valueField:'dicId',textField:'dicName',url:'<%=path %>/dataDictionaryController/findByMark?mark=title',required:true,editable:false" />
		</td>
	</tr>	
	...
	<tr>
		<td>教研室</td>
		<td>
			<input name="teaTroom" id="teaTroom" type="text" class="easyui-combobox"    
    			data-options="valueField:'troomId',textField:'troomName',url:'<%=path %>/troomController/findTroomsByAcademy',required:true,editable:false" />
		</td>
		<td>院系</td>
		<td>
